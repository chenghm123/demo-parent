package com.accelerator.demo.standalone.zookeeper.lock;
...
public class DistributedLock implements Watcher {
    ...
    private static final String CONNECTION_STRING = "zookeeper:2181";
    ...
    public boolean createPath(String path, String data, boolean needWatch) throws KeeperException, InterruptedException {
        if (zk.exists(path, needWatch) == null) {
            LOG.info(LOG_PREFIX_OF_THREAD + "节点创建成功, Path: " + this.zk.create(path, data.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT) + ", content: " + data);
        }
        return true;
    }
